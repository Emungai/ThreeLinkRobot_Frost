...
%% Impact Code
guard = RigidImpact('SwingLegImpact',domain,'leftFootHeight');
...
system = addVertex(system,'RightStance','Domain',domain);%'Control', io_control) %adding a node
    %'RightStance': the names of the vertices to be added
...
srcs = {'RightStance'}; %source node
tars = {'RightStance'}; %target node
...
nlp = HybridTrajectoryOptimization('ThreeLink_system',system,num_grid,[]);
setBounds;
...
nlp.Phase(1).removeConstraint('u_leftFootHeight_Swing');
%% Create Ipopt solver
addpath(genpath(export_path));
nlp.update;
